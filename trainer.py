...
class Trainer:
    def __init__(self, options):
        ...
        self.models["encoder"] = networks.ResnetEncoder(  ## depth net
        ...
        if self.use_pose_net:
            if self.opt.pose_model_type == "separate_resnet":
                self.models["pose_encoder"] = networks.ResnetEncoder(    ## pose net
                    self.opt.num_layers,
...
######  load mono
        self.models_mono["encoder"] = networks.ResnetEncoder(  ## depth net
            self.opt.num_layers, self.opt.weights_init == "pretrained")
        self.models_mono["encoder"].to(self.device)
        self.parameters_to_train += list(self.models_mono["encoder"].parameters())

        self.models_mono["depth"] = networks.DepthDecoder(
            self.models_mono["encoder"].num_ch_enc, self.opt.scales)
        self.models_mono["depth"].to(self.device)



        self.parameters_to_train += list(self.models_mono["depth"].parameters())  ##### only train this


        if self.use_pose_net:
            if self.opt.pose_model_type == "separate_resnet":
                self.models_mono["pose_encoder"] = networks.ResnetEncoder(    # pose net
                    self.opt.num_layers,
                    self.opt.weights_init == "pretrained",
                    num_input_images=self.num_pose_frames)

                self.models_mono["pose_encoder"].to(self.device)
                self.parameters_to_train += list(self.models_mono["pose_encoder"].parameters())

                self.models_mono["pose"] = networks.PoseDecoder(
                    self.models_mono["pose_encoder"].num_ch_enc,
                    num_input_features=1,
                    num_frames_to_predict_for=2)

            self.models_mono["pose"].to(self.device)
            self.parameters_to_train += list(self.models_mono["pose"].parameters())
        ...
        if self.opt.predictive_mask:
            ...
            self.models_mono["predictive_mask"] = networks.DepthDecoder(
                self.models_mono["encoder"].num_ch_enc, self.opt.scales,
                num_output_channels=(len(self.opt.frame_ids) - 1))
            self.models_mono["predictive_mask"].to(self.device)
            self.parameters_to_train += list(self.models_mono["predictive_mask"].parameters())
    ...
    def set_train(self):
        ...
        for m in self.models_mono.values():  
            m.train()
    ...
    def process_batch(self, inputs):
        ...
        features_mono = self.models_mono["encoder"](inputs["color_aug", 0, 0])
        outputs_mono = self.models_mono["depth"](features_mono)


        if self.use_pose_net:
            outputs.update(self.predict_poses(inputs, features))
            outputs_mono.update(self.predict_poses(inputs, features_mono))


        self.generate_images_pred(inputs, outputs)
        self.generate_images_pred(inputs, outputs_mono)
        losses = self.compute_losses(inputs, outputs, outputs_mono) 
    ...
    def compute_losses(self, inputs, outputs, outputs_mono):  ####### ori is compute_losses(self, inputs, outputs):
        ...
        for scale in self.opt.scales:
            ...
            disp_mono = outputs_mono[("disp", scale)]  #####
            ...
            for frame_id in self.opt.frame_ids[1:]:
                ...
                equal_losses.append(self.compute_reprojection_loss(outputs[("disp", 0)], outputs_mono[("disp", 0)]))



            reprojection_losses = torch.cat(reprojection_losses, 1)

            equal_losses = torch.cat(equal_losses, 1)  ######
            ...
            if self.opt.avg_reprojection:
                ...
                equal_loss = equal_losses.mean(1, keepdim=True)  ####
            else:
                # differently to Monodepth2, compute mins as we go
                reprojection_loss, _ = torch.min(reprojection_losses, dim=1, keepdim=True)

                equal_loss, _ = torch.min(equal_losses, dim=1, keepdim=True)  ####
            ...
            loss += reprojection_loss + depth_hint_loss + 0.5 * equal_loss  #####
    ...
    def load_model_mono(self):
        ...
        for n in self.opt.models_to_load:
            print("Loading {} weights...".format(n))
            path = os.path.join(self.opt.load_weights_folder, "{}.pth".format(n))
            model_dict = self.models_mono[n].state_dict()
            pretrained_dict = torch.load(path)
            pretrained_dict = {k: v for k, v in pretrained_dict.items() if k in model_dict}
            model_dict.update(pretrained_dict)
            self.models_mono[n].load_state_dict(model_dict)
